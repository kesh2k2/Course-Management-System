...
class CourseClass:
    ...
    def add(self):
        ...
        if self.var_course.get() == "":
            ...
            try:
                ...
                if row is not None:
                    messagebox.showerror("Error", "This course title already exists", parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            ...
            course_id = self.CourseTable.item(selected_item)["values"][0]
    ...
    def delete(self):
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM course WHERE id=%s", (course_id,))
            self.conn.commit()

            if self.cursor.rowcount == 0:
                messagebox.showerror("Error", "Failed to delete course. It may not exist.", parent=self.root)
            else:
    ...
    def search(self):
        self.connect_db()
        try:
            self.cursor.execute("SELECT * FROM course WHERE name LIKE %s", ('%' + self.var_search.get() + '%',))
            rows = self.cursor.fetchall()
            self.CourseTable.delete(*self.CourseTable.get_children())
            for row in rows:
                self.CourseTable.insert('', END, values=row)
